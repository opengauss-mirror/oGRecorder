...
do
    ...
    ## rand
    #head -c 24 /dev/urandom > $certPath/$role.key.rand
    ## cipher
    #echo "$passwd" | openssl enc -aes-256-cbc -salt -in $certPath/$role.key -out $certPath/$role.key.cipher -pass stdin

    #rm -f $certPath/$role.csr
done
