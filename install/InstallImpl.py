...
class InstallImpl:
    ...
    def AddWrResource(self):
        """
        Add wr Resource
        """
        self.logger.log("Add wr resource.")
        cmd = "source %s ; sh %s/wr_res.sh" % (self.envFile, self.toolPath)
        for host in self.hostnames:
            status, output = self.executeCmdOnHost(host, cmd)
            if status != 0:
                self.logger.debug("Command: " + cmd)
                errorDetail = "\nStatus: %s\nOutput: %s" % (status, output)
                self.logger.logExit(("Failed to add wr resource to %s." % host) + errorDetail)
