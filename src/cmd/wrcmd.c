...
status_t wr_load_buffer_cache_group_from_file(
    int32 file_fd, int64 *length, const char *vg_name, wr_vg_info_item_t *vg_item)
{
    uint32 group_num = 0;
    int64 offset = *length;
    int32 read_size = 0;
    ...
    WR_RETURN_IF_ERROR(wr_get_group_num(file_fd, &offset, &group_num));
    char read_vg_name[WR_MAX_NAME_LEN];
    uint32 i;
    bool32 find = CM_FALSE;
    for (i = 0; i < group_num; i++) {
        status = cm_read_file(file_fd, read_vg_name, WR_MAX_NAME_LEN, &read_size);
        WR_RETURN_IFERR2(status, LOG_DEBUG_ERR("Failed to read file."));
        offset += WR_MAX_NAME_LEN;
        if (strcmp(vg_name, read_vg_name) != 0) {
            uint64 bucket_size = WR_MAX_SEGMENT_NUM * (uint32)sizeof(uint32_t);
            ...
            WR_RETURN_IF_FALSE2(result, LOG_DEBUG_ERR("Failed to seek file %d", file_fd));
            continue;
        }
        WR_RETURN_IF_ERROR(wr_load_buffer_cache_from_file(file_fd, vg_item, &offset));
        find = CM_TRUE;
    }
    if (!find) {
    ...
    WR_RETURN_IF_ERROR(wr_load_buffer_pool_from_file(file_fd, GA_8K_POOL));
    WR_RETURN_IF_ERROR(wr_load_buffer_pool_from_file(file_fd, GA_16K_POOL));
    WR_RETURN_IF_ERROR(wr_load_buffer_pool_from_file(file_fd, GA_FS_AUX_POOL));
    WR_RETURN_IF_ERROR(wr_load_buffer_pool_from_file(file_fd, GA_SEGMENT_POOL));
    return CM_SUCCESS;
}

status_t wr_load_wr_ctrl_group_from_file(
    int32 file_fd, int64 *length, const char *vg_name, wr_vg_info_item_t *vg_item)
{
    uint32 group_num = 0;
    int64 offset = 0;
    ...
    for (i = 0; i < group_num; i++) {
        WR_RETURN_IF_FALSE2(wr_check_software_version(file_fd, &offset),
            LOG_DEBUG_ERR("Failed to check software_version of vg %u", i));
        status = cm_read_file(file_fd, read_vg_name, WR_MAX_NAME_LEN, &read_size);
        WR_RETURN_IFERR2(status, LOG_DEBUG_ERR("Failed to read file."));
        offset += WR_MAX_NAME_LEN;
        if (strcmp(vg_name, read_vg_name) != 0) {
            offset += (int64)sizeof(wr_ctrl_t);
            result = (bool32)(cm_seek_file(file_fd, offset, SEEK_SET) != -1);
            WR_RETURN_IF_FALSE2(result, LOG_DEBUG_ERR("Failed to seek file %d", file_fd));
            continue;
        }
        WR_RETURN_IF_ERROR(wr_load_wr_ctrl_from_file(file_fd, vg_item));
        offset += (int64)sizeof(wr_ctrl_t);
        find = CM_TRUE;
    }
    if (!find) {
