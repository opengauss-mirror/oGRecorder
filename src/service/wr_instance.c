...
static bool32 wr_find_unreg_volume(wr_session_t *session, char **dev, uint8 *vg_idx, uint8 *volume_id)
{
    for (uint32 i = 0; i < g_vgs_info->group_num; i++) {
        for (uint32 j = 0; j < WR_MAX_VOLUMES; j++) {
            if (g_vgs_info->volume_group[i].wr_ctrl->volume.defs[j].flag != VOLUME_PREPARE) {
                continue;
            }
            wr_lock_vg_mem_and_shm_s(session, &g_vgs_info->volume_group[i]);
            if (g_vgs_info->volume_group[i].wr_ctrl->volume.defs[j].flag != VOLUME_PREPARE) {
                wr_unlock_vg_mem_and_shm(session, &g_vgs_info->volume_group[i]);
                continue;
            }
            *dev = g_vgs_info->volume_group[i].wr_ctrl->volume.defs[j].name;
            *vg_idx = (uint8)i;
            *volume_id = (uint8)j;
            wr_unlock_vg_mem_and_shm(session, &g_vgs_info->volume_group[i]);
            return CM_TRUE;
...
static bool32 wr_is_register(iof_reg_in_t *reg_info, int64 host_id)
{
...
void wr_check_unreg_volume(wr_session_t *session)
{
    uint8 vg_idx, volume_id;
    iof_reg_in_t reg_info;
    (void)memset_s(&reg_info, sizeof(reg_info), 0, sizeof(reg_info));

    bool32 is_unreg = wr_find_unreg_volume(session, &reg_info.dev, &vg_idx, &volume_id);
    if (!is_unreg) {
        return;
    }
    ...
    wr_vg_info_item_t *vg_item = &g_vgs_info->volume_group[0];
    if (wr_lock_vg_storage_r(vg_item, vg_item->entry_path, g_inst_cfg) != CM_SUCCESS) {
        return;
    }
    wr_lock_vg_mem_and_shm_s(session, vg_item);
    ret = wr_load_vg_ctrl_part(vg_item, (int64)(WR_VOLUME_HEAD_SIZE - WR_DISK_UNIT_SIZE),
        &vg_item->wr_ctrl->global_ctrl, WR_DISK_UNIT_SIZE, &remote);
    wr_unlock_vg_mem_and_shm(session, vg_item);
    (void)wr_unlock_vg_storage(vg_item, vg_item->entry_path, g_inst_cfg);
    if (ret != CM_SUCCESS) {
        return;
    }

    bool32 is_reg = CM_FALSE;
    for (uint8 i = 0; i < WR_MAX_INSTANCES; i++) {
        is_reg = cm_bitmap64_exist(&vg_item->wr_ctrl->global_ctrl.cluster_node_info, i);
        if (is_reg && !wr_is_register(&reg_info, i)) {
            return;
        }
    }
    ...
    if (vg_item->wr_ctrl->volume.defs[volume_id].flag == VOLUME_FREE) {
        wr_unlock_vg_mem_and_shm(session, vg_item);
        return;
    }
    vg_item->wr_ctrl->volume.defs[volume_id].flag = VOLUME_OCCUPY;
    ret = wr_update_volume_ctrl(vg_item);
    wr_unlock_vg_mem_and_shm(session, vg_item);
    if (ret != CM_SUCCESS) {
        return;
    }
...
void wr_meta_syn_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        // WR_STATUS_OPEN for control with switchover
        if (g_wr_instance.status == WR_STATUS_OPEN) {
            (void)wr_meta_syn(session, bg_task_info);
        }
...
void wr_recycle_meta_proc(thread_t *thread)
{
    ...
    while (!thread->closed) {
        // WR_STATUS_OPEN for control with switchover
        if (g_wr_instance.status == WR_STATUS_OPEN) {
            wr_recycle_meta(session, bg_task_info, &clean_time);
            cm_sleep(CM_SLEEP_10_FIXED);
            continue;
        }
        cm_sleep(CM_SLEEP_10_FIXED);
