...
void wr_proc_normal_ack(wr_session_t *session, mes_msg_t *msg)
{
    ...
    LOG_DEBUG_INF("[MES] Receive ack(%u),src inst(%u), dst inst(%u).", (uint32_t)(wr_head->wr_cmd),
...
static inline wr_remote_ack_hdl_t *wr_get_remote_ack_handle(int32_t cmd)
{
...
static void wr_init_mes_head(wr_message_head_t *head, uint32_t cmd, uint32_t flags, uint16 src_inst, uint16 dst_inst,
...
static void wr_ack_version_not_match(wr_session_t *session, wr_message_head_t *req_head, uint32_t version)
{
    wr_config_t *inst_cfg = wr_get_inst_cfg();
    wr_params_t *param = &inst_cfg->params;
    uint16 dst_inst = req_head->src_inst;
    ...
    uint32_t cmd = (req_head->wr_cmd == WR_CMD_REQ_BROADCAST) ? WR_CMD_ACK_BROADCAST_WITH_MSG : WR_CMD_ACK_SYB2ACTIVE;
...
static void wr_set_cluster_proto_vers(uint8 inst_id, uint32_t version)
{
    ...
    do {
        uint32_t cur_version = (uint32_t)cm_atomic32_get((atomic32_t *)&g_wr_instance.cluster_proto_vers[inst_id]);
        if (cur_version == version) {
            break;
        }
        set_flag = cm_atomic32_cas(
            (atomic32_t *)&g_wr_instance.cluster_proto_vers[inst_id], (int32_t)cur_version, (int32_t)version);
    } while (!set_flag);
}
...
static int wr_handle_recv_broadcast_msg(
    ruid_type ruid, uint32_t timeout, uint64 *succ_ack_inst, wr_recv_msg_t *recv_msg_output)
{
...
uint32_t wr_get_broadcast_proto_ver(uint64 succ_inst)
{
    ...
    uint32_t broadcast_proto_vers = WR_PROTO_VERSION;
    for (uint32_t i = 0; i < WR_MAX_INSTANCES; i++) {
        inst_mask = ((uint64)0x1 << i);
        if ((need_send_inst & inst_mask) == 0) {
            continue;
        }
        inst_proto_ver = (uint32_t)cm_atomic32_get((atomic32_t *)&g_wr_instance.cluster_proto_vers[i]);
...
void wr_get_valid_inst(uint64 valid_inst, uint32_t *arr, uint32_t count)
{
    uint32_t i = 0;
    for (uint32_t j = 0; j < WR_MAX_INSTANCES; j++) {
...
static status_t wr_broadcast_msg_with_try(wr_message_head_t *wr_head, wr_recv_msg_t *recv_msg, unsigned int timeout)
{
    ...
    do {
        ...
        uint32_t valid_inst_arr[WR_MAX_INSTANCES] = {0};
...
static status_t wr_broadcast_msg(char *req_buf, uint32_t size, wr_recv_msg_t *recv_msg, unsigned int timeout)
{
...
static status_t wr_prepare_ack_msg(
    wr_session_t *session, status_t ret, char **ack_buf, uint32_t *ack_size, uint32_t version)
{
    int32_t code;
...
static status_t wr_process_remote_req_prepare(wr_session_t *session, mes_msg_t *msg, wr_processor_t *processor)
{
    ...
    if (wr_head->wr_cmd != WR_CMD_REQ_BROADCAST &&
        ...
        LOG_RUN_ERR("Proc msg cmd:%u from remote node:%u fail, can NOT exec here.", (uint32_t)wr_head->wr_cmd,
            wr_head->src_inst);
        return CM_ERROR;
    }
    ...
    if (wr_check_srv_status(msg) != CM_TRUE) {
        LOG_RUN_WAR("Proc msg cmd:%u from remote node:%u fail, local status %u not open, wait try again.",
            (uint32_t)wr_head->wr_cmd, wr_head->src_inst, g_wr_instance.status);
        return CM_ERROR;
    }
    return CM_SUCCESS;
}
...
static void wr_process_message(uint32_t work_idx, ruid_type ruid, mes_msg_t *msg)
{
    cm_reset_error();
    ...
    wr_message_head_t *wr_head = (wr_message_head_t *)msg->buffer;
    LOG_DEBUG_INF("[MES] Proc msg cmd:%u, src node:%u, dst node:%u begin.", (uint32_t)(wr_head->wr_cmd),
        (uint32_t)(wr_head->src_inst), (uint32_t)(wr_head->dst_inst));
    ...
    if (wr_head->msg_proto_ver > WR_PROTO_VERSION) {
        uint32_t curr_proto_ver = MIN(wr_head->sw_proto_ver, WR_PROTO_VERSION);
    ...
    LOG_DEBUG_INF("[MES] Proc msg cmd:%u, src node:%u, dst node:%u end.", (uint32_t)(wr_head->wr_cmd),
        (uint32_t)(wr_head->src_inst), (uint32_t)(wr_head->dst_inst));
}
...
static void wr_set_group_task_num(wr_config_t *wr_profile, mes_profile_t *mes_profile)
{
    uint32_t work_thread_cnt_load_meta =
        (uint32_t)(wr_profile->params.work_thread_cnt * WR_WORK_THREAD_LOAD_DATA_PERCENT);
    if (work_thread_cnt_load_meta == 0) {
        work_thread_cnt_load_meta = 1;
    }
    uint32_t work_thread_cnt_comm = (wr_profile->params.work_thread_cnt - work_thread_cnt_load_meta);
    ...
    mes_profile->recv_task_count[MES_PRIORITY_ZERO] =
        MAX(1, (uint32_t)(work_thread_cnt_load_meta * WR_RECV_WORK_THREAD_RATIO));
    ...
    mes_profile->recv_task_count[MES_PRIORITY_ONE] =
        MAX(1, (uint32_t)(work_thread_cnt_comm * WR_RECV_WORK_THREAD_RATIO));
}
...
status_t wr_notify_expect_bool_ack(wr_vg_info_item_t *vg_item, wr_bcast_req_cmd_t cmd, uint64 ftid, bool32 *cmd_ack)
{
    ...
    do {
        ...
        if (ret == ERR_WR_VERSION_NOT_MATCH) {
            uint32_t new_proto_ver = wr_get_broadcast_proto_ver(recv_msg.succ_inst);
...
status_t wr_exec_sync(wr_session_t *session, uint32_t remoteid, uint32_t currtid, status_t *remote_result)
{
    ...
    wr_config_t *inst_cfg = wr_get_inst_cfg();
    uint32_t timeout = inst_cfg->params.mes_wait_timeout;
    uint32_t new_proto_ver = wr_get_version(&session->recv_pack);
    do {
        uint32_t buf_size = WR_MES_MSG_HEAD_SIZE + session->recv_pack.head->size;
    ...
    *remote_result = ack_head->result;
    uint32_t body_size = ack_head->size - WR_MES_MSG_HEAD_SIZE;
...
status_t wr_exec_on_remote(uint8 cmd, char *req, int32_t req_size, char *ack, int ack_size, status_t *remote_result)
{
    status_t ret = CM_ERROR;
    ...
    do {
        uint32_t proto_ver = wr_get_remote_proto_ver(remoteid);
        // 1. init msg head
    ...
    *remote_result = ack_head->result;
    LOG_DEBUG_INF("[MES] wr server receive msg from remote node, cmd:%u, ack to cli data size:%hu, remote_result:%u.",
        ack_head->wr_cmd, ack_head->size, (uint32_t)*remote_result);
    if (*remote_result != CM_SUCCESS) {
        if (ack_head->size < sizeof(wr_remote_exec_fail_ack_t)) {
            WR_THROW_ERROR(ERR_WR_MES_ILL, "msg len is invalid");
...
void wr_proc_syb2active_req(wr_session_t *session, mes_msg_t *msg)
{
    ...
    uint32_t size = req_head.size - WR_MES_MSG_HEAD_SIZE;
