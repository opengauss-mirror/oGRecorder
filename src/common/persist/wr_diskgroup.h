...
#define WR_READ4STANDBY_ERR (int32)3
...
status_t wr_read_volume_inst(
    wr_vg_info_item_t *vg_item, wr_volume_t *volume, int64 offset, void *buf, int32 size, bool32 *remote);
status_t wr_init_vol_handle(wr_vg_info_item_t *vg_item, int32 flags, wr_vol_handles_t *vol_handles);
void wr_destroy_vol_handle(wr_vg_info_item_t *vg_item, wr_vol_handles_t *vol_handles, uint32 size);
...
static inline uint32 wr_get_checksum(void *data, uint32 len)
{
    char *buf = (char *)data;
...
static inline void wr_check_checksum(uint32 checksum0, uint32 checksum1)
{
...
static inline bool32 wr_read_remote_checksum(void *buf, int32 size)
{
    uint32 sum1 = *(uint32 *)buf;
    uint32 sum2 = wr_get_checksum(buf, (uint32)size);
...
static inline void wr_set_vg_au_size(wr_ctrl_t *ctrl, uint32 au_size)
{
    CM_ASSERT(au_size <= WR_MAX_AU_SIZE);
    ctrl->core.au_size = au_size;
}

static inline bool32 wr_check_volume_is_used(wr_vg_info_item_t *vg_item, uint32 vid)
{
...
status_t wr_check_remove_volume(wr_vg_info_item_t *vg_item, const char *volume_name, uint32 *volume_id);
void wr_remove_volume_vg_ctrl(wr_ctrl_t *vg_ctrl, uint32 id);
