...
typedef struct st_wr_core_ctrl {
    uint32 checksum;  // NOTE:checksum can not change the position in the struct.wr_get_checksum need.
    uint32 reserve;
    uint64 version;
...
typedef struct st_wr_redo_ctrl {
    ...
    uint32 redo_size[WR_MAX_EXTENDED_COUNT];  // except redo_size > 32KB
    uint32 count;
    char reserve[376];
} wr_redo_ctrl_t;
...
static inline void wr_set_software_version(wr_vg_header_t *vg_header, uint32 version)
{
    CM_ASSERT(vg_header != NULL);
    vg_header->software_version = version;
}

static inline uint32 wr_get_software_version(wr_vg_header_t *vg_header)
{
    CM_ASSERT(vg_header != NULL);
