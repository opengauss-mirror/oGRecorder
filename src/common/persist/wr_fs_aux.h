...
void wr_get_inited_size_with_fs_aux(
    wr_vg_info_item_t *vg_item, wr_fs_aux_t *fs_aux, int64 offset, int32_t size, int32_t *inited_size);

status_t wr_try_find_data_au_batch(wr_session_t *session, wr_vg_info_item_t *vg_item, gft_node_t *node,
    wr_fs_block_t *second_block, uint32_t block_au_count_beg);
...
status_t wr_read_volume_with_fs_aux(wr_vg_info_item_t *vg_item, gft_node_t *node, wr_fs_aux_t *fs_aux,
    wr_volume_t *volume, int64 vol_offset, int64 offset, void *buf, int32_t size);
...
status_t wr_get_fs_aux_with_cache(wr_session_t *session, wr_vg_info_item_t *vg_item, gft_node_t *node,
    wr_block_id_t block_id, uint32_t block_au_count, wr_fs_aux_t **fs_aux_out);
...
status_t wr_update_fs_aux_bitmap2disk(wr_vg_info_item_t *item, wr_fs_aux_t *block, uint32_t size, bool32 had_checksum);
#ifdef __cplusplus
}
#endif
