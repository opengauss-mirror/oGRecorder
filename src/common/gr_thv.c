...
#ifndef WIN32
static __thread gr_thv_run_ctx_t gr_thv_run_ctx = {0};
#else
__declspec(thread) gr_thv_run_ctx_t gr_thv_run_ctx = {0};
#endif
...
{
    if (gr_thv_run_ctx.thread_id != 0) {
        return gr_thv_run_ctx.thread_id;
    }
    gr_thv_run_ctx.thread_id = cm_get_current_thread_id();
    return gr_thv_run_ctx.thread_id;
}

void gr_set_thv_run_ctx_item(gr_thv_run_ctx_item_e item, void *item_addr)
{
    if (item < GR_THV_RUN_CTX_ITEM_MAX) {
        gr_thv_run_ctx.item_addr[item] = item_addr;
    }
}
