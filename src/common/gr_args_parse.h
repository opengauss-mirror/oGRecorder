 ...
 *
 * IDENTIFICATION
 *    src/common/gr_args_parse.h
 *
 * -------------------------------------------------------------------------
 */

#ifndef GR_ARGS_PARSE_H_
#define GR_ARGS_PARSE_H_
...
typedef status_t (*cmd_parse_check_t)(gr_args_t *cmd_args_set, int set_size);
typedef struct st_gr_args_set_t {
    gr_args_t *cmd_args;
    int32_t args_size;
    cmd_parse_check_t args_check;
} gr_args_set_t;

typedef void (*gr_admin_help)(const char *prog_name, int print_flag);
typedef status_t (*gr_admin_cmd_proc)(void);
typedef struct st_gr_admin_cmd_t {
    char cmd[CM_MAX_NAME_LEN];
    gr_admin_help help;
    gr_admin_cmd_proc proc;
    gr_args_set_t *args_set;
    bool8 log_necessary;  // Logs are necessary for commands which write disks, and unnecessary for others.
} gr_admin_cmd_t;

typedef enum en_gr_help_type {
    GR_HELP_DETAIL = 0,
    GR_HELP_SIMPLE,
} gr_help_type;

#define GR_ARG_IDX_0 0
#define GR_ARG_IDX_1 1
#define GR_ARG_IDX_2 2
#define GR_ARG_IDX_3 3
#define GR_ARG_IDX_4 4
#define GR_ARG_IDX_5 5
#define GR_ARG_IDX_6 6
#define GR_ARG_IDX_7 7
#define GR_ARG_IDX_8 8
#define GR_ARG_IDX_9 9
#define GR_ARG_IDX_10 10
...
status_t gr_check_meta_type(const char *type);
status_t gr_check_meta_id(const char *intput);
