...
void *cm_native_attach_shm(cm_shm_handle_t handle, uint32_t flag)
{
    ...
    for (uint32_t i = 0; i < retry_num; i++) {
        ...
        if ((int64)result == -1) {
            ...
            offset = ((uint64)result) % GR_ALIGN_SIZE;
            if (offset == 0) {
                return result;
            } else {
                shmdt(result);
                result = (char *)result + (GR_ALIGN_SIZE - offset) + GR_ALIGN_SIZE * retry_num;
            }
        }
    }
    return NULL;
...
void *cm_do_attach_shm_without_register(cm_shm_key_t key, uint64 size, uint32_t flag, bool32 logging_open_err)
{
    ...
    if (entry->handle == CM_INVALID_SHM_HANDLE) {
        if (logging_open_err) {
            GR_LOG_WITH_OS_MSG("Failed to open shared memory, key=0x%08x, size=%llu", key, size);
        }
        return NULL;
    } else {
...
static status_t cm_check_shm_ctrl(void)
{
    ...
    if (cm_native_shm_size(CM_SHM_CTRL_KEY) != CM_SHM_SIZE_OF_CTRL) {
        GR_THROW_ERROR(ERR_GR_SHM_CHECK, CM_SHM_CTRL_KEY, "mismatched size");
        return CM_ERROR;
    }
#endif
...
static bool32 cm_do_detach_shm(cm_shm_key_t key, bool32 logging_err)
{
    ...
    if (cm_native_detach_shm(addr)) {
        ...
        if (logging_err) {
            GR_LOG_WITH_OS_MSG("Failed to detach shared memory,key=0x%08x", key);
        }
        return CM_FALSE;
    }
}
...
static bool32 cm_del_shm_block(cm_shm_key_t key)
{
    if (!do_del_shm_directly(key)) {
        GR_LOG_WITH_OS_MSG("Failed to delete shared memory,key=0x%08x", key);
        return CM_FALSE;
    }
