...
status_t shm_hashmap_move_bucket_node(
    wr_session_t *session, shm_hash_ctrl_t *hash_ctrl, uint32 old_bucket_idx, uint32 new_bucket_idx)
{
...
status_t wr_hashmap_redistribute(wr_session_t *session, shm_hash_ctrl_t *hash_ctrl, uint32 old_bucket)
{
...
{
    uint32 segment = (hash_ctrl->max_bucket + 1) / WR_BUCKETS_PER_SEGMENT;
    ...
    ga_obj_id_t obj_id, char *meta_addr, wr_block_type_t type)
{
    wr_block_ctrl_t *block_ctrl = WR_GET_BLOCK_CTRL_FROM_META(meta_addr);
    shm_hash_ctrl_t *hash_ctrl = &vg_item->buffer_cache->hash_ctrl;
    uint32 hash = WR_BUFFER_CACHE_HASH(block_id);
    ...
    wr_vg_info_item_t *vg_item, wr_block_id_t block_id, char *buf, int64_t offset, int32 size, bool32 calc_checksum)
{
    ...
    status_t status = wr_check_read_volume(vg_item, (uint32)block_id.volume, offset, buf, size, &remote);
    if (status != CM_SUCCESS) {
        return status;
    }
    ...
    if ((calc_checksum) && (remote == CM_FALSE)) {
        cm_panic((uint32)size == WR_BLOCK_SIZE || (uint32)size == WR_FILE_SPACE_BLOCK_SIZE ||
                 (uint32)size == WR_FS_AUX_SIZE);
        uint32 checksum = wr_get_checksum(buf, (uint32)size);
    ...
    char *meta_addr, bool32 *is_changed, bool32 force_refresh)
{
    ...
    if (wr_compare_version(disk_version, version) || force_refresh) {
        WR_LOG_DEBUG_OP(
            ...
            version, disk_version, wr_display_metaid(block_id), type, (uint32)force_refresh);
        ...
        if (size == WR_DISK_UNIT_SIZE) {
            ...
            if (force_refresh && version == 0) {
                status = wr_get_block_from_disk(vg_item, block_id, meta_addr, offset, (int32)size, CM_FALSE);
            } else {
                status = wr_get_block_from_disk(vg_item, block_id, meta_addr, offset, (int32)size, CM_TRUE);
            }
            if (status != CM_SUCCESS) {
    ...
    wr_block_type_t type, char **block_addr, ga_obj_id_t *out_obj_id)
{
    ...
    status_t status = wr_get_block_from_disk(vg_item, block_id, meta_addr, offset, (int32)size, CM_TRUE);
    if (status != CM_SUCCESS) {
        wr_unlock_shm_meta_bucket(session, &bucket->enque_lock);
...
void *wr_find_block_in_bucket(wr_session_t *session, wr_vg_info_item_t *vg_item, uint32 hash, uint64 *key,
    bool32 is_print_error_log, ga_obj_id_t *out_obj_id)
{
    ...
    auid_t block_id_tmp = {0};
    uint32 bucket_idx = shm_hashmap_calc_bucket_idx(hash_ctrl, hash);
    uint32 segment_objid = WR_INVALID_ID32;
...
static void *wr_find_block_in_bucket_ex(wr_session_t *session, wr_vg_info_item_t *vg_item, uint32 hash, uint64 *key,
    bool32 is_print_error_log, ga_obj_id_t *out_obj_id)
{
    ...
    shm_hashmap_bucket_t *bucket, auid_t add_block_id, wr_block_type_t type, char *refresh_buf, char **shm_buf)
{
    ...
    uint32 size = wr_buffer_cache_get_block_size(type);
    wr_block_ctrl_t *block_ctrl = NULL;
    uint32 hash = WR_BUFFER_CACHE_HASH(add_block_id);
    uint32 obj_id = ga_alloc_object(pool_id, CM_INVALID_ID32);
    if (obj_id == CM_INVALID_ID32) {
...
static status_t wr_refresh_buffer_cache_inner(wr_session_t *session, wr_vg_info_item_t *vg_item, uint32 bucket_idx,
...
status_t wr_refresh_buffer_cache(wr_session_t *session, wr_vg_info_item_t *vg_item, shm_hashmap_t *map)
{
    ...
    for (uint32 i = 0; i < WR_BLOCK_TYPE_MAX; i++) {
...
static uint32 wr_try_find_recycle_meta_by_bucket(wr_session_t *session, wr_vg_info_item_t *vg_item,
...
static inline uint32 wr_recycle_meta_batch_num(bool32 trigger_enable)
{
    ...
    wr_recycle_meta_args_t *recycle_meta_args, bool32 trigger_enable)
{
    ...
    for (; (bucket_id < cur_map_num && bucket_id < hash_ctrl->max_bucket); bucket_id++) {
        ...
        uint32 batch_num = wr_recycle_meta_batch_num(trigger_enable);
