...
static void gr_simulation_init()
{
...
void gr_simulation_cm_refresh(void)
{
    ...
    if (g_simulation_cm.params.lock_owner_id < GR_MIN_INST_ID || g_simulation_cm.params.lock_owner_id >= GR_MAX_INST_ID) {
...
static void gr_simulation_cm_thread(thread_t *thread)
{
    ...
    while (!thread->closed) {
        ...
        if (status != CM_SUCCESS) {
            ...
            cm_sleep(GR_LONG_TIMEOUT);
            continue;
        }
        gr_simulation_cm_refresh();
        cm_spin_unlock(&g_simulation_cm.lock);
        cm_sleep(GR_LONG_TIMEOUT);
    }
}

status_t gr_simulation_cm_lsnr(void)
{
    ...
    LOG_RUN_INF("[GR][simulation_cm]gr_simulation_cm thread started");
    gr_simulation_init();
    int ret = cm_create_thread(gr_simulation_cm_thread, 0, g_cm_config_realpath, &g_simulation_cm.thread);
    if (ret != CM_SUCCESS) {
        LOG_RUN_ERR("[GR][simulation_cm]fail to create gr_simulation_cm_thread");
        return CM_ERROR;
    }
    return ret;
}

status_t gr_refresh_cm_config_lock_owner_id(unsigned int inst_id)
{
...
status_t gr_simulation_cm_init(unsigned int instance_id, const char *res_name, cm_notify_func_t func)
{
    status_t status = gr_simulation_cm_lsnr();
    if(status == CM_SUCCESS) {
...
char *gr_simulation_cm_get_res_stat(void)
{
...
void gr_simulation_cm_free_res_stat(char *res_stat)
{
    if (res_stat != NULL) {
        cm_free(res_stat);
    }
...
status_t gr_simulation_cm_res_lock(const char *lock_name)
{
    ...
    if (g_simulation_cm.params.lock_owner_id == CM_INVALID_ID32) {
        return gr_refresh_cm_config_lock_owner_id(curr_id);
    }
    return CM_SUCCESS;
}

status_t gr_simulation_cm_res_unlock(const char *lock_name)
{
    gr_config_t *inst_cfg = gr_get_inst_cfg();
    uint32_t curr_id = (uint32_t)inst_cfg->params.inst_id;
    LOG_RUN_INF("[GR][simulation_cm]Simulate to unlock %s.", lock_name);
    if (g_simulation_cm.params.lock_owner_id == curr_id) {
        return gr_refresh_cm_config_lock_owner_id(CM_INVALID_ID32);
    }
    return CM_SUCCESS;
}

status_t gr_simulation_cm_res_get_lock_owner(const char *lock_name, unsigned int *inst_id)
{
...
status_t gr_simulation_cm_res_trans_lock(const char *lock_name, unsigned int inst_id)
{
    LOG_RUN_INF("[GR][simulation_cm]Simulate to trans lock %s.", lock_name);
    return gr_refresh_cm_config_lock_owner_id(inst_id);
}

#define CM_SIMULATION_PATH_LEN 10
status_t gr_simulation_cm_res_mgr_init(const char *so_lib_path, cm_res_mgr_t *cm_res_mgr, cm_allocator_t *alloc)
{
    ...
    cm_res_mgr->cm_init = gr_simulation_cm_init;
    cm_res_mgr->cm_get_res_stat = gr_simulation_cm_get_res_stat;
    cm_res_mgr->cm_free_res_stat = gr_simulation_cm_free_res_stat;
    cm_res_mgr->cm_res_lock = gr_simulation_cm_res_lock;
    cm_res_mgr->cm_res_unlock = gr_simulation_cm_res_unlock;
    cm_res_mgr->cm_res_get_lock_owner = gr_simulation_cm_res_get_lock_owner;
    cm_res_mgr->cm_res_trans_lock = gr_simulation_cm_res_trans_lock;
    return CM_SUCCESS;
}

void gr_simulation_cm_res_mgr_uninit(cm_res_mgr_t *cm_res_mgr)
{
