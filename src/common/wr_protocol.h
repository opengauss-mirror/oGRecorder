...
#define WR_CMD_TYPE_OFFSET(cmd_id) ((uint32)(cmd_id) - (uint32)WR_CMD_BEGIN)
...
#define WR_INVALID_VERSION (int32)0x7FFFFFFF
...
static inline void wr_init_packet(wr_packet_t *pack, uint32 options)
{
    CM_ASSERT(pack != NULL);
    pack->offset = 0;
...
static inline void wr_set_client_version(wr_packet_t *pack, uint32 version)
{
    CM_ASSERT(pack != NULL);
    pack->head->client_version = version;
}

static inline void wr_set_version(wr_packet_t *pack, uint32 version)
{
    CM_ASSERT(pack != NULL);
    pack->head->version = version;
}

static inline uint32 wr_get_client_version(wr_packet_t *pack)
{
    CM_ASSERT(pack != NULL);
    return pack->head->client_version;
}

static inline uint32 wr_get_version(wr_packet_t *pack)
{
    CM_ASSERT(pack != NULL);
...
static inline void wr_init_get(wr_packet_t *pack)
{
    ...
    pack->offset = (uint32)sizeof(wr_packet_head_t);
}

static inline void wr_init_set(wr_packet_t *pack, uint32 proto_version)
{
    if (pack == NULL) {
        return;
    }
    (void)memset_s(pack->head, sizeof(wr_packet_head_t), 0, sizeof(wr_packet_head_t));
    pack->head->size = (uint32)sizeof(wr_packet_head_t);
...
static inline status_t wr_put_data(wr_packet_t *pack, const void *data, uint32 size)
{
    errno_t errcode = 0;
...
static inline status_t wr_put_int32(wr_packet_t *pack, uint32 value)
{
    CM_ASSERT(pack != NULL);

    *(uint32 *)WR_WRITE_ADDR(pack) = (CS_DIFFERENT_ENDIAN(pack->options) != 0) ? cs_reverse_int32(value) : value;
    pack->head->size += (uint32)sizeof(uint32);
    return CM_SUCCESS;
}

static inline status_t wr_reserv_text_buf(wr_packet_t *pack, uint32 size, char **data_buf)
{
    CM_ASSERT(pack != NULL);
    CM_ASSERT(data_buf != NULL);
    if (CM_ALIGN4(size) >= WR_REMAIN_SIZE(pack) - sizeof(uint32)) {
        CM_THROW_ERROR(ERR_BUFFER_OVERFLOW, size, WR_REMAIN_SIZE(pack) - 1);
        return CM_ERROR;
    }

    // record the size first
    *(uint32 *)WR_WRITE_ADDR(pack) = (CS_DIFFERENT_ENDIAN(pack->options) != 0) ? cs_reverse_int32(size) : size;
    pack->head->size += (uint32)sizeof(uint32);
...
static inline status_t wr_pack_check_len(wr_packet_t *pack, uint32 inc)
{
...
static inline status_t wr_get_data(wr_packet_t *pack, uint32 size, void **buf)
{
    int64 len;
    char *temp_buf = NULL;
    CM_ASSERT(pack != NULL);
...
static inline status_t wr_get_int32(wr_packet_t *pack, int32 *value)
{
    int32 temp_value;
    CM_ASSERT(pack != NULL);

    CM_RETURN_IFERR(wr_pack_check_len(pack, sizeof(int32)));

    temp_value = *(int32 *)WR_READ_ADDR(pack);
...
static inline status_t wr_get_text(wr_packet_t *pack, text_t *text)
{
    ...
    CM_RETURN_IFERR(wr_get_int32(pack, (int32 *)&text->len));
    if ((text->len > WR_MAX_PACKET_SIZE) || (text->len == 0)) {
