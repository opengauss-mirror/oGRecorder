...
status_t cmd_realpath_home(const char *input_args, char **convert_result, int *convert_size)
{
    ...
    *convert_result = (char *)malloc(CM_FILE_NAME_BUFFER_SIZE);
    ...
    if (status != CM_SUCCESS) {
        WR_PRINT_ERROR("path is insecure, home: %s.\n", input_args);
        free(*convert_result);
        *convert_result = NULL;
        return status;
    }
...
status_t cmd_check_convert_wr_home(const char *input_args, void **convert_result, int *convert_size)
{
    if (input_args == NULL) {
        *convert_result = NULL;
        *convert_size = 0;
    ...
    if (status != CM_SUCCESS) {
        WR_PRINT_ERROR("home realpth failed, home: %s.\n", input_args);
        return status;
    }
    return CM_SUCCESS;
}
...
status_t cmd_check_uint64(const char *str)
{
    ...
    if (status == CM_ERROR) {
        WR_PRINT_ERROR("%s is not a valid uint64\n", str);
        return CM_ERROR;
    }
    return CM_SUCCESS;
}
